"""
Modal NCU profiling script for persistent_kernel2.cu (v2 — split routing + persistent GEMM)

Compiles the CUDA kernel on a remote GPU, runs it under NVIDIA Nsight Compute (ncu),
and downloads the .ncu-rep file locally for analysis.

Usage:
    modal run modal_ncu.py                         # default: 64 tokens, B200
    modal run modal_ncu.py --tokens 128
    modal run modal_ncu.py --output my_trace.ncu-rep

Open locally with:
    ncu-ui moe_ncu_profile.ncu-rep
"""

import modal
from pathlib import Path

app = modal.App("persistent-moe-ncu")

# CUDA 12.x dev container with nvcc and ncu
# Copy local .cu and .h source files into the image
image = (
...
void fill_fp8(void* d_ptr, size_t n) {
    // Allocate host, fill with small floats, convert to FP8, copy
    float* h = (float*)malloc(n * sizeof(float));
    for (size_t i = 0; i < n; i++)
        h[i] = 0.01f * ((float)(rand() % 201 - 100));  // [-1.0, 1.0]

    // Convert float -> FP8 on host
...
int main(int argc, char** argv) {
    ...
    printf("persistent_kernel2 NCU harness: T=%d\n", T);
...
def profile_ncu(tokens: int = 64) -> bytes:
    import os
    import subprocess
    ...
    # B200 reports compute_cap 10.0 → sm_100, but tcgen05/TMA bulk require sm_100a
    if arch == "100":
        arch = "100a"
    print(f"Detected GPU compute capability: sm_{arch}")

    # Compile: persistent_kernel2.cu + test_harness.cu
    # No -rdc/-lcudadevrt needed — v2 uses regular kernel launch (no cooperative groups)
    compile_cmd = [
        "nvcc",
        "-o", "moe_test",
        "persistent_kernel2.cu", "test_harness.cu",
        "-gencode", "arch=compute_100,code=sm_100a",
        "-Xptxas", "--gpu-name=sm_100a",
        "-std=c++17",
        "-O2",
        "--expt-relaxed-constexpr",
        "-lineinfo",           # source correlation in ncu
    ]
    ...
    # Run under ncu
    # v2 launches 3 kernels per call: routing_topk, routing_permute, persistent_gemm
    # We do warmup + profiled run = 6 kernel launches total
    # Skip first 3 (warmup), profile next 3 (profiled run)
    ncu_output = "/tmp/moe_ncu_profile_v2"
    ncu_cmd = [
        "ncu",
        "--set", "full",                     # collect all metrics
        "--launch-skip", "3",                # skip warmup (3 kernels)
        "--launch-count", "3",               # profile all 3 kernels in profiled run
        "-o", ncu_output,
        "--force-overwrite",
        "./moe_test", str(tokens),
    ]
    print("Profiling:", " ".join(ncu_cmd))
    r = subprocess.run(ncu_cmd, capture_output=True, text=True)
    print("NCU STDOUT:", r.stdout[-2000:] if len(r.stdout) > 2000 else r.stdout)
    if r.stderr:
        print("NCU STDERR:", r.stderr[-2000:] if len(r.stderr) > 2000 else r.stderr)

    rep_file = ncu_output + ".ncu-rep"
    # ncu may append a numeric suffix — check for that too
    if not os.path.exists(rep_file):
        import glob
        candidates = sorted(glob.glob(ncu_output + "*.ncu-rep"))
        if candidates:
            rep_file = candidates[-1]
    if not os.path.exists(rep_file):
        raise RuntimeError(
            f"ncu did not produce {rep_file}. Return code: {r.returncode}"
        )
...
def main(
    tokens: int = 64,
    output: str = "moe_ncu_profile_v2.ncu-rep",
):
    print(f"Launching NCU profiling on Modal B200 (T={tokens}) ...")
    data = profile_ncu.remote(tokens=tokens)

    with open(output, "wb") as f:
        f.write(data)

    size_mb = len(data) / 1024 / 1024
    print(f"\nProfile saved to '{output}'  ({size_mb:.1f} MB)")
    print("Open locally with:")
    print(f"  ncu-ui {output}")
